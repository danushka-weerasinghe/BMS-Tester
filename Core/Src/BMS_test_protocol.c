 ...
 */
  ...
  #include "config.h"
  #include "modbus_crc.h"  // For crc16 function
  #include "modbusSlave.h"  // For sendData function

  // Forward declarations
  extern void sendData(uint8_t *data, int size);
...
void tester_setup(void)
{
 ...
 //                    	Voltage_Sequence_Automatic();
                 ...
                 case 0x05:  // DC-CSU-Volt (Range: 1-23, Value: 2.0-4.2V)
                         ...
                         // Map 1..23 cell IDs to 0-based index per IC: 1..12 -> 0..11, 13..24 -> 0..11
                         if (CELL_ID >= 13){
                             CELL_ID_2 = (uint8_t)(CELL_ID - 13);
                             BMS_IC_NUM  = 1;
                         } else {
                             CELL_ID_2 = (uint8_t)(CELL_ID - 1);
                         }
 ...
 //                        uint8_t CELL_ID 	= RxData_modbus_01[4];
                         ...
                         if (CELL_ID_2 < BMS_IC[BMS_IC_NUM].ic_reg.cell_channels) {
                             TxData_modbus_01[4] = (uint8_t)(BMS_IC[BMS_IC_NUM].cells.c_codes[CELL_ID_2] >> 8);
                             TxData_modbus_01[5] = (uint8_t)(BMS_IC[BMS_IC_NUM].cells.c_codes[CELL_ID_2] & 0xFF);
                         } else {
                             TxData_modbus_01[4] = 0;
                             TxData_modbus_01[5] = 0;
                         }
                 ...
                 case 0x06:  // DC-CSU-Temp (Range: 1-6, Value: -20 to 100°C)
                     if (id >= 1 && id <= 6)
                     {
                         ...
                         TxData_modbus_01[4] = BMS_IC[BMS_IC_NUM].aux.a_codes[TEMP_ID] >> 8;
                         TxData_modbus_01[5] = BMS_IC[BMS_IC_NUM].aux.a_codes[TEMP_ID] & 0xFF;
 ...
 void Voltage_Sequence_Automatic(void)
 {
     ...
     const float test_voltages[] = {4.0f};//{2.0f, 2.5f, 2.8f, 3.3f, 3.4f, 3.6f, 4.0f, 4.2f};
 ...
 //    HAL_Delay(10);
  ...
  float Get_INA_Voltage(const Cell_Config* cell)
 {
