...
void LTC6811_set_discharge(int Cells, //The cells to be discharged
        uint8_t Current_ic, //Number of ICs in the system
        cell_asic *ic //A two dimensional array that will store the data
        )
{
    if (Cells == 0)
    {
        ...
        ic[Current_ic].config.tx_data[5] = ic[Current_ic].config.tx_data[5]
                & 0xF0;
    }
    else
    {
        ic[Current_ic].config.tx_data[4] = 0x00;
        ic[Current_ic].config.tx_data[4] = Cells;
        ic[Current_ic].config.tx_data[5] &=0xF0;
        ic[Current_ic].config.tx_data[5] |= ((Cells >> 8) & 0x0F);
    }
}
