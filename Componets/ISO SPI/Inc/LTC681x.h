...
#ifndef SOURCECODE_ANALOG_LTC681X_H_
#define SOURCECODE_ANALOG_LTC681X_H_
...
typedef struct
{
    ...
    uint16_t max_cell_v_local;
    uint16_t min_cell_v_local;
    uint8_t max_cell_id_local :4;
    uint8_t min_cell_id_local :4;
} cv;
...
{
    int8_t temp[16];
    int8_t highest_t_local;
    int8_t lowest_t_local;
    uint8_t highest_t_id_local;
    uint8_t lowest_t_id_local;
} tsv;
...
typedef struct
{
    ...
    ic_register sctrl;
    ic_register sctrlb;
    uint8_t sid[6];bool isospi_reverse;
...
typedef struct
{
    uint16_t ov_threshold_ic;
    uint16_t uv_threshold_ic;
    uint16_t balancing_start_volt_ic;
    uint8_t balancing_max_vgap;
    uint8_t balancing_timer :4;
    uint8_t GPIO_setup :5;
    uint8_t Ref_on :1;
    uint8_t ADC_mode :1;
    uint8_t ADC_frequency :2;
    uint8_t discharge_permision :1;
} setup_data_sv;

typedef struct
{
    uint16_t cell_discharge :12;
} control_data_sv;
/*!
...
#endif /* SOURCECODE_ANALOG_LTC681X_H_ */
